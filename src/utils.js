...
function validaTelefone(telefone) {
    ...
    // Aceita 10 dígitos (fixo) ou 11 dígitos (celular com 9º dígito)
    return telefoneLimpo.length === 10 || telefoneLimpo.length === 11;
}

// Função para validar telefone com 9º dígito (celular)
function validaTelefoneCelular(telefone) {
    const telefoneLimpo = telefone.replace(/\D/g, '');
    // Deve ter 11 dígitos e o terceiro dígito deve ser 9
    return telefoneLimpo.length === 11 && telefoneLimpo[2] === '9';
}

// Função para aplicar trim e validar tamanho mínimo
